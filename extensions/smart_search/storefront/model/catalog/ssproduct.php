...
class ModelCatalogSsproduct extends Model{
  /**
  ...
  public function getProductsByKeyword($keyword, $category_id = 0, $description = false, $model = false, $sort = 'p.sort_order', $order = 'ASC', $start = 0, $limit = 20){
        ...
        $lower_search_keyword = strtolower($keyword);
        while (($csv_keywords = fgetcsv($keyword_list)) !== FALSE) {
          // $lower_line = array_map('strtolower', $line[0]);
          $lower_csv_keywords = (strtolower($csv_keywords[0]));
          // echo $lower_csv_keywords;
          // echo "<br>";
          if (strpos($lower_csv_keywords, $lower_search_keyword) !== false) {
             $split_terms = $csv_keywords[1];
             break;
          }
          // if (in_array($lower_keyword, $lower_line))
          // {
          //   $split_terms = $line[1];
          //   break;
          // }
      ...
      if ($split_terms){
          ...
          // $query = "SELECT * FROM abc_products p WHERE p.sku IN (CONCAT(\"'\", REPLACE(`RCS5T, RCB3P`, `,`, `','`), \"'\"))";


          $split_terms = "'".str_replace(',', "','", $split_terms)."'";
          $split_terms = str_replace(' ','',$split_terms);

          // $query = "SELECT DISTINCT p.product_id,p.model,p.sku,p.location,p.quantity,p.stock_checkout,p.stock_status_id,p.manufacturer_id,p.shipping,p.ship_individually,p.free_shipping,p.shipping_price,p.price,p.tax_class_id,p.date_available,p.weight,p.weight_class_id,p.length,p.width,p.height,p.length_class_id,p.status,p.viewed,p.sort_order,p.subtract,p.minimum,p.maximum,p.cost,p.call_to_order,p.settings,p.date_added,p.date_modified,pd.language_id,pd.name,pd.meta_keywords,pd.meta_description,pd.description,pd.blurb,p2s.store_id,m.manufacturer_id,m.name,m.sort_order,ss.stock_status_id,ss.language_id,ss.name,pt.tag,pt.language_id, ".$this->_sql_final_price_string().", pd.name, pd.blurb, m.name AS manufacturer, ss.name AS stock, 
          // (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
              // " . $this->_sql_avg_rating_string() . ",
              // " . $this->_sql_review_count_string() . "
              // " . $this->_sql_join_string() . "
              //     LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
              //     WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (p.sku IN (".$split_terms.")  OR p.product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%' ) UNION SELECT related_id FROM " . $this->db->table("products_related") . " WHERE product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%'))))";


          $query = "SELECT *, p.product_id, ".$this->_sql_final_price_string().", pd.name AS name, pd.blurb, m.name AS manufacturer, ss.name AS stock, 
           (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
               " . $this->_sql_avg_rating_string() . ",
               " . $this->_sql_review_count_string() . "
               " . $this->_sql_join_string() . "
                   LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
                   WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (p.sku IN (".$split_terms."))";
          // $query = "SELECT *, p.product_id, ".$this->_sql_final_price_string().", pd.name AS name, pd.blurb, m.name AS manufacturer, ss.name AS stock, 
          //  (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
          //      " . $this->_sql_avg_rating_string() . ",
          //      " . $this->_sql_review_count_string() . "
          ...
          //          LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
          //          WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (p.sku IN (".$split_terms.")  OR p.product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%' ) UNION SELECT related_id FROM " . $this->db->table("products_related") . " WHERE product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%'))))";
          // $query .= " AND p.status = '1' AND p.date_available <= NOW() GROUP BY p.product_id";
          ...
          $query .= ", (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) DESC";
          ...
          if ($sql->num_rows){
            foreach ($sql->rows as $value){
              ...
              $products1[$value['product_id']] = $value;
            }
          }      
      }
      //trim keyword
      ...
      $pipe_keyword = str_replace(' ', '|', $keyword);
      $products_id = implode(',', $product_id);
      $products_id = "'".str_replace(',', "','", $products_id)."'";
      if ($pipe_keyword) {
        
      $product_title_query = "SELECT  *, p.product_id, ".$this->_sql_final_price_string().", pd.name, pd.blurb, m.name AS manufacturer, ss.name AS stock, 
                (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
                    " . $this->_sql_avg_rating_string() . ",
                    " . $this->_sql_review_count_string() . "
                    " . $this->_sql_join_string() . "
                        LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
                        WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ((pd.name REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]') OR (pd.blurb REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]')) AND p.product_id NOT IN (".$products_id.")";
                        
      // $product_title_query .= " AND p.status = '1' AND p.date_available <= NOW() GROUP BY p.product_id";
      ...
      if (isset($sort) && in_array($sort, array_keys($sort_data))){
        $product_title_query .= " ORDER BY " . $sort_data[$sort];
      } else{
        $product_title_query .= " ORDER BY p.sort_order";
      }

      if ($order == 'DESC'){
        $product_title_query .= " DESC";
      } else{
        $product_title_query .= " ASC";
      }

      $product_title_query .= ", (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) DESC";
      ...
      $product_title_query .= " LIMIT " . (int)$start . "," . (int)$limit;
      // print_r($product_title_query);die;
                      
      
      $sql2 = $this->db->query($product_title_query);
      $products2 = array (); 
      if ($sql2->num_rows){
        foreach ($sql2->rows as $value2){
          $product_id[] = $value2['product_id'];
          $products2[$value2['product_id']] = $value2;
        }
      }
      $combine_product_id = $product_id;
      $combine_product_id = implode(',', $combine_product_id);
      $combine_product_id = "'".str_replace(',', "','", $combine_product_id)."'";

      $product_description_query = "SELECT  *,p.product_id, ".$this->_sql_final_price_string().", pd.name, pd.blurb, m.name AS manufacturer, ss.name AS stock, 
                (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
                    " . $this->_sql_avg_rating_string() . ",
                    " . $this->_sql_review_count_string() . "
                    " . $this->_sql_join_string() . "
                        LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
                        WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (pd.description REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]') AND p.product_id NOT IN (".$combine_product_id.")";
    // $product_description_query .= " AND p.status = '1' AND p.date_available <= NOW() GROUP BY p.product_id";
      ...
      if (isset($sort) && in_array($sort, array_keys($sort_data))){
        $product_description_query .= " ORDER BY " . $sort_data[$sort];
      } else{
        $product_description_query .= " ORDER BY p.sort_order";
      }

      if ($order == 'DESC'){
        $product_description_query .= " DESC";
      } else{
        $product_description_query .= " ASC";
      }

      $product_description_query .= ", (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) DESC";
      ...
      $product_description_query .= " LIMIT " . (int)$start . "," . (int)$limit;
     
      $sql3 = $this->db->query($product_description_query);
          $products3 = array ();
          if ($sql3->num_rows){
            foreach ($sql3->rows as $value3){
              $products3[$value3['product_id']] = $value3;
            }
          }
      $products_array = array('products' =>array('prod1' => $products1, 'prod2' => $products2, 'prod3' => $products3 )); 
    ...
    //   if (!$description){
    //     $sql .= " AND (LCASE(pd.name) REGEXP '" . $this->db->escape(mb_strtolower($keyword),true) . "' OR " . implode(' OR ', $tags_str);
    //   } else{
    //     $sql .= " AND (LCASE(pd.name) REGEXP '" . $this->db->escape(mb_strtolower($keyword),true) . "'
    //             OR " . implode(' OR ', $tags_str) . "
    //             OR LCASE(pd.description) REGEXP '" . $this->db->escape(mb_strtolower($keyword),true) . "'";
    //   }

    //   if (!$model){
    //     $sql .= ")";
    //   } else{
    //     $sql .= " OR LCASE(p.model) REGEXP '" . $this->db->escape(mb_strtolower($keyword),true) . "')";
    //   }
    //   $category_sql = "SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name = '" . $this->db->escape(mb_strtolower($category_keyword),true) . "' LIMIT 1 ";
    ...
    //   $sql .= ", (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) DESC";
  ...
  public function getTotalProductsByKeyword($keyword, $category_id = 0, $description = false, $model = false){
    ...
    if ($pipe_keyword){
      ...
      $tags = explode(' ', trim($pipe_keyword));
      $tags_str = array ();
      if (sizeof($tags) > 1) $tags_str[] = " LCASE(pt.tag) = '" . $this->db->escape(trim($pipe_keyword)) . "' ";
      foreach ($tags as $tag){
        $tags_str[] = " LCASE(pt.tag) = '" . $this->db->escape(mb_strtolower($tag)) . "' ";
      }

      if (!$description){
        $sql .= " AND (LCASE(pd.name) REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]')  OR " . implode(' OR ', $tags_str);
      } else{
        $sql .= " AND (LCASE(pd.name) REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]') OR " . implode(' OR ', $tags_str) . " OR LCASE(pd.description) REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]')";
      }

      if (!$model){
        $sql .= "";
      } else{
        $sql .= " OR LCASE(p.model) REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]'))";
      }
      ...
      $query = $this->db->query($sql);
      if ($query->num_rows){
        $description_total = $query->row['total'];
    ...
    ////Blurb Title query
      ...
      $blurb_section_query = "SELECT  COUNT( DISTINCT p.product_id ) AS total, ".$this->_sql_final_price_string().",
          (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
              " . $this->_sql_avg_rating_string() . ",
              " . $this->_sql_review_count_string() . "
              " . $this->_sql_join_string() . "
                  LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
                  WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (pd.name REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]') OR (pd.blurb REGEXP '[[:<:]](".$pipe_keyword.")[[:>:]]')";
      $section_query = $this->db->query($blurb_section_query);
      if ($section_query->num_rows){
        $blurb_total = $section_query->row['total'];
        // return $query->row['total'];
      } else{
        return 0;
      }
      ...
      if ($split_terms){
       ...
       $csv_section_query = "SELECT COUNT( DISTINCT p.product_id) as total, ".$this->_sql_final_price_string().",
          (SELECT COUNT(*) AS numrows FROM " . $this->db->table("order_products") . " AS product_quant WHERE product_quant.product_id = p.product_id) AS total_sales,
               " . $this->_sql_avg_rating_string() . ",
               " . $this->_sql_review_count_string() . "
               " . $this->_sql_join_string() . "
                   LEFT JOIN " . $this->db->table("product_tags") . " pt ON (p.product_id = pt.product_id)
                   WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (p.sku IN (".$split_terms.")  OR p.product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%' ) UNION SELECT related_id FROM " . $this->db->table("products_related") . " WHERE product_id IN (SELECT product_id FROM " . $this->db->table("products_to_categories") . " WHERE category_id IN (SELECT category_id FROM " . $this->db->table("category_descriptions") . " WHERE name LIKE '%".$keyword."%'))))";
      $related_section_query = $this->db->query($csv_section_query);
      if ($related_section_query->num_rows){
        $csv_total = $related_section_query->row['total'];
      } 
      }
      if ($csv_total > $blurb_total && $csv_total > $description_total) {
        return $csv_total;
      }elseif ($blurb_total > $csv_total && $blurb_total > $description_total) {
        return $blurb_total;
      }elseif ($description_total > $csv_total && $description_total > $blurb_total) {
        return $description_total;
      }else{
        return 0;
      }
    } else{
  ...
  public function insertKeywords($keyword){
    $sql = "INSERT INTO " . $this->db->table("search_log") . " (search_keyword) VALUES ('".$keyword."')";
    $query = $this->db->query($sql);
    return $query;
  }
}
